...
const BlogDetails = () => {
    ...
    // get blog data
    const { data: blog, error, isPending } = useQuery({
    ...
    // comment post
    const { isPending: isPendingData, error:postError, data: postData, mutate } = useMutation({

        mutationFn: async () => {
            if (emty) {
                return;
            }
            const commentPost = await axiosApi.post(CommentUrl, postComment)
    ...
    const { data: commentData, isPending: commentIsPending } = useQuery({
        queryKey: ['commentData'],
        queryFn: async () => {
            const fetchData = await axiosApi.get(allCommentUrl)
            return fetchData.data
        },
        initialData: []
    })

console.log(commentData,commentIsPending)
    console.log(postData, isPendingData, error,postError)
            ...
            <div className="max-w-4xl mx-auto p-5 bg-white">
                ...
                <div className="flex gap-5 py-5">
                    ...
                    <form onSubmit={handleForm} className="w-full">
                        ...
                        <p>{emptyField ? emptyField : ''}</p>
                        <input className="bg-[#F1F2F2] hover:bg-[#ebf0f0] px-7  py-1 my-3 rounded-3xl border-blue-100 border font-semibold " type="submit" value={isPendingData ? "Loading.." : "Submit"} id="" />
                    </form>
                    {/* Comment section */}
